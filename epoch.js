(function () {
	...
	if( typeof module !== 'undefined' && module.exports ) {
		...
		Epoch.prototype.path = __dirname;

		Epoch.prototype.load = function( path ) {
		...
		Epoch.prototype.path = [].pop
			.call( document.getElementsByTagName( 'script' ) ).src.split( "/" )
			.slice( 0, -1 ).join( "/" );
		root.epoch = epoch;


		Epoch.prototype.load = function( url ) {
			...
			if (xhr.status !== 200) {
				...
				eval( xhr.responseText )( Epoch );
			}
		};
	}


	Epoch.prototype.version = '0.0.10';

	Epoch.prototype.strftime = function( str ) {
		this.load( this.path + "/epoch.strftime.js" );

		// loading the strftime lib causes this function to replace itself, so call it again
		this.strftime(str);
	},
	...
	Epoch.prototype.clear = function() {
		// this._d = ( format ? new Date(format) : new Date() );
	...
	// Epoch.prototype.zero = function( num, size ) {
	// 	var s = String(num);
	...
	// Epoch.prototype.modify = function( set, dSet, dGet ) {
	// 	dSet( ( /(\+|-)\d/g.exec( set )
	// 					? dGet() + parseInt( set )
	// 					: set - 1 ) );
	// },
